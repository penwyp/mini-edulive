...
func (s *QuicServer) Start(ctx context.Context, connectionHandler func(quic.Connection)) {
	for {
		...
		case <-ctx.Done():
			...
			return
		default:
			acceptCtx, span := ob.StartSpan(ctx, "QuicServer.Accept")
			startTime := time.Now()
			...
			if err != nil {
				...
				ob.RecordError(span, err, "QuicServer.Accept")
			...
			ob.RecordLatency(acceptCtx, "quic.Accept", time.Since(startTime))
			span.End()
...
func (s *QuicServer) ReadInitialMessage(ctx context.Context, stream quic.Stream) (*protocol.BulletMessage, error) {
	// 读取原始数据
	data, err := s.ReadMessage(ctx, stream)
	if err != nil {
		return nil, err
	}
...
func (s *QuicServer) ReadMessage(ctx context.Context, stream quic.Stream) ([]byte, error) {
	ctx, span := ob.StartSpan(ctx, "QuicServer.ReadMessage")
	defer span.End()
