...
type Config struct {
	...
	Gateway       Gateway       `mapstructure:"gateway"` // 网关配置
...
type Gateway struct {
	WorkerThread      int `mapstructure:"workerThread"`      // 工作线程数
	MessageBufferSize int `mapstructure:"messageBufferSize"` // 消息缓冲区大小
}

func (i Gateway) GetWorkerThread() int {
	if i.WorkerThread <= 0 {
		return 20
	}
	return i.WorkerThread
}

func (i Gateway) GetMessageBufferSize() int {
	if i.MessageBufferSize <= 0 {
		return 10000
	}
	return i.MessageBufferSize
}
...
type Prometheus struct {
	...
	Port         int    `mapstructure:"port"`        // 仅 gateway/worker/dispatcher 使用
	RemoteWrite  string `mapstructure:"remoteWrite"` // 新增远程写入端点，例如 "http://127.0.0.1:8490/api/v1/write"
}

// Jaeger 追踪配置
type Jaeger struct {
	Enabled      bool    `mapstructure:"enabled"`
	UIEndpoint   string  `mapstructure:"uiendpoint"`
	HttpEndpoint string  `mapstructure:"httpEndpoint"`
...
func setDefaultValues(v *viper.Viper) {
	...
	v.SetDefault("gateway.workerThread", 20)
	v.SetDefault("gateway.messageBufferSize", 10000)
	...
	v.SetDefault("observability.prometheus.port", 8320)
	v.SetDefault("observability.prometheus.remoteWrite", "http://127.0.0.1:8490/api/v1/write")

	v.SetDefault("observability.jaeger.enabled", false)
	v.SetDefault("observability.jaeger.uiendpoint", "127.0.0.1:8430")
...
func validateClient(cfg *Config) error {
	...
	if cfg.Observability.Prometheus.Enabled && cfg.Observability.Prometheus.RemoteWrite == "" {
		return fmt.Errorf("prometheus remoteWrite endpoint cannot be empty when enabled")
	}
	return nil
}
